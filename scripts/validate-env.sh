...
paperclip_enabled="$(env_value PAPERCLIP_ENABLED)"
...
if [[ "$mode" == "coolify" ]]; then
  missing=0
  if truthy "$paperclip_enabled"; then
    for pair in \
  ...
  if truthy "$hermes_dashboard_enabled"; then
...
if truthy "$paperclip_enabled"; then
  ...
  echo "Services: hermes (paperclip disabled)"
fi
if [[ "$mode" == "coolify" ]]; then
  if truthy "$paperclip_enabled"; then

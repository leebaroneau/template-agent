...
build_hermes_archive() {
  ...
  docker exec "$HERMES_CONTAINER" bash -lc 'cd /data || exit 2; tar czf /tmp/hermes-profiles.tar.gz --exclude="hermes/profiles/*/repos" --exclude="hermes/profiles/*/sessions" --exclude="hermes/profiles/*/profile-backups" --exclude="hermes/profiles/*/python-packages" --exclude="hermes/profiles/*/bin" --exclude="hermes/profiles/*/lsp" --exclude="hermes/profiles/*/cache" --exclude="hermes/profiles/*/audio_cache" --exclude="*/__pycache__" hermes/profiles hermes/SOUL.md hermes/auth.json hermes/.env hermes/cron hermes/hooks 2>/dev/null; rc=$?; [ "$rc" -le 1 ]'
